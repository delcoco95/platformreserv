...
export const appointmentService = {
  ...
  onClientAppointmentsChange(clientId: string, callback: (appointments: Appointment[]) => void) {
    const q = query(
      collection(db, 'appointments'),
      where('clientId', '==', clientId),
      orderBy('date', 'desc')
  ...
  onProfessionalAppointmentsChange(professionalId: string, callback: (appointments: Appointment[]) => void) {
    const q = query(
      collection(db, 'appointments'),
      where('professionalId', '==', professionalId),
      orderBy('date', 'desc')
